...
static void
call_my_boot_entry(struct segoff_s bootsegip, u8 bootdrv)
{
    dprintf(1, "Booting from %04x:%04x\n", bootsegip.seg, bootsegip.offset);
    struct bregs br;
    memset(&br, 0, sizeof(br));
...
static void
boot_my_disk(u8 bootdrv, int checksig)
{
    u16 bootseg = 0x07c0;
    ...
    if (checksig) {
        ...
        if (GET_FARVAR(bootseg, mbr->signature) != 0x1BC7) {
    ...
    call_my_boot_entry(SEGOFF(bootseg, bootip), bootdrv);
}
...
handle_19(void)
{
    printf("handle int 19...\n");
    debug_enter(NULL, DEBUG_HDL_19);
    BootSequence = 0;
    do_boot(0);
}

// INT 60h Boot from another magic 
void VISIBLE32FLAT
handle_60(void) {
    printf("handle int 60...\n");
    boot_my_disk(0x80, 1);
}
